...
function LoginForm() {
  ...
  const redirectTo = searchParams.get('redirectTo') || '/design';
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      console.error('Google login error:', error);
      showToast(t('auth.login.errors.networkError'), 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Validation
    if (!email) {
      showToast(t('auth.login.errors.emailRequired'), 'error');
      return;
    }
    if (!password) {
      showToast(t('auth.login.errors.passwordRequired'), 'error');
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('Invalid login credentials')) {
          ...
          showToast(t('auth.login.errors.emailNotConfirmed'), 'error');
        } else if (error.message.includes('Email link is invalid or has expired')) {
          showToast(t('auth.login.errors.linkExpired'), 'error');
        } else {
          showToast(error.message, 'error');
        }
        return;
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  />
                  <label htmlFor="remember" className={`${direction === 'rtl' ? 'mr-2' : 'ml-2'} text-sm text-neutral-700`}>
                    {t('auth.login.rememberMe')}
                  </label>
                </div>
                <Link href="/auth/reset-password" className="text-sm text-primary hover:text-primary-dark transition-colors">
